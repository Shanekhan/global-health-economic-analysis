...
# ==========================================
# Yeh line current folder ka rasta nikal legi
current_dir = os.path.dirname(os.path.abspath(__file__))

def get_file_path(filename):
    # Pehle root mein check karega, phir data/processed mein
...
with tab_exec:
    ...
    st.divider()
    fig_map = px.choropleth(latest_record, locations="Country Name", locationmode="country names", color="Mortality_Rate", color_continuous_scale="Viridis", template="plotly_dark", height=500)
...
with tab_sim:
    ...
    if st.button("RUN PREDICTIVE CLASSIFICATION"):
        input_matrix = {'GDP_per_capita': gdp_val, 'Health_Expenditure': health_val, 'Food_Price_Index': food_val, 'Homicide_Rate': base_data.get('Homicide_Rate', 2.0), 'Food_Price_Lag1': food_val * 1.05, 'GDP_Lag1': gdp_val * 0.95, 'GDP_Growth_Rate': 0.02, 'Health_Efficiency_Index': health_val / (base_data['Mortality_Rate'] + 1)}
